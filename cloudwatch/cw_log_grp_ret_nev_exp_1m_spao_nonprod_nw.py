...
RETENTION_DAYS = 30 #Enter the no of retention days
...
# Function to set the retention policy for log groups that don't have it set already
    ...
    try:
        ...
        for page in paginator.paginate():
            for log_group in page['logGroups']:
                ...
                # Check if the retention policy is set (None means no retention is set)
                if 'retentionInDays' not in log_group or log_group['retentionInDays'] is None:
                    logger.info(
                        f"Profile {profile} - Setting retention for log group '{log_group_name}' to {RETENTION_DAYS} days (no retention set previously)."
                    )
                    ...
                    logger.info(
                        f"Profile {profile} - Log group '{log_group_name}' already has retention set to {log_group['retentionInDays']} days. Skipping."
                    )
